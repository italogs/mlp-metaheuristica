...
#
	...
	print GPL_LIN_FILE <<EOF;
		...
		plot "$emp_lin_filename" t "empirical" w points, "$the_lin_filename" t "estimated" with lines linetype 3, "$up_lin_filename" t "+1 std dev range" w lines linetype 4, "$lo_lin_filename" t "-1 std dev range" w lines linetype 4 
		quit
EOF
...
#
	...
	system("gnuplot $gpl_lin_filename") == 0 ||
                     die "1-gnuplot (needed for plotting) not found \n";
...
#
	...
	print GPL_EXP_FILE <<EOF;
		...
		plot "$emp_exp_filename" t "empirical" w points, "$the_exp_filename" t "theoretical" w lines linetype 3
		quit
EOF
...
#
	...
	system("gnuplot $gpl_exp_filename") == 0 ||
                     die "2-gnuplot (needed for plotting) not found \n";
